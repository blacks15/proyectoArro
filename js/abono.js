...
/////////////////////////////////////////////////////////////////////////////
	...
	**************************/
	...
	$("#buscarE").on('keypress',function(evt){
		...
		//EVENTO KEYPRESS
	$("#abono").on('keypress',function(evt){
		var charCode = evt.which || evt.keyCode;
	...
	$("#abono").on('keyup',function(){
		var abono = $(this).val();
		var saldoAnterior = $("#saldoAnterior").text();
		var saldoNuevo = "";

		if (abono != "") {
			if (abono > 0) {
				if (abono > parseInt(saldoAnterior)) {
					global.mensajes('Advertencia','El abono no puede ser mayor que la deuda','info');
					abono.substring(0,abono.length-1);
					$(this).val(abono);
					$("#saldoNuevo").html(saldoAnterior);
					$("#btnSave").prop('disabled',true);
				} else {
					saldoNuevo = parseInt(saldoAnterior) - parseInt(abono);
					$("#saldoNuevo").html(saldoNuevo);
					$("#btnSave").prop('disabled',false);
				}
			} else {
				$("#saldoNuevo").html(saldoAnterior);
				$("#btnSave").prop('disabled',true);
				global.mensajes('Advertencia','El abono debe ser mayor a cero','info');
			}
		} else {
			$("#btnSave").prop('disabled',true);
		}
	...
	$('#tableDeudas').on('click',function() {
		...
		$(".btnMostrar").on('click',function(index){
			...
			if (respuesta.codRetorno == '000') {
				...
				saldoAnterior = array[2].replace('$','');
				$("#folioVenta").html(array[0]);
				$("#saldoAnterior").html(saldoAnterior);
				$("#abono").focus();
					//LEVANTAMOS LA MODAL
				$('#modal').modal('open');
			}
