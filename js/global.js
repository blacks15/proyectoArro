var global = {
                    ...
                    if (data.form == 'Libro') {
                        ...
                        $.each(data.datos, function(index, value) {
                            $("#table").append("<tr><td style='display: none' width='5%'>" + value.id +
                                ...
                                "</td><td style='display: none' width='5%'>" + value.rutaIMG +
                        ...
                        $.each(data.datos, function(index, value) {
                            $("#table").append("<tr><td style='display: none' width='5%'>" + value.id +
                                "</td><td width='15%'>" + value.folio + "</td><td width='25%'>" + value.nombreEmpleado +
        ...
        $.ajax({
            ...
            success: function(response) {
                if (response.codRetorno == '000') {
                    ...
                    global.mensajes('Error', response.Mensaje, 'warning', '', '', '', '');
                } else if (response.codRetorno == '005') {
                    global.mensajes('Advertencia', response.Mensaje, 'warning', '', '', response.form, response.bus);
                }
            },
            error: function(xhr, ajaxOptions, throwError) {
                if (xhr.status == 404) {
    ...
    cargaFolio: function(url, opc, tipo) {
        return respuesta = global.buscar(url, opc, tipo, '');
    },
